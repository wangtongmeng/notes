...
let p = new Person();
let man = new Man('男', 'zhansan');
let women = new Women('女', 'lisi');
p.buy();
man.buy();
women.buy();
...
let zhansan = {
  // marrry(p: Women) {
  marrry(p: Person) {
    console.log(`张三和${p.name}`);
    
  }
}

// zhansan.marrry(new Women('女', '范冰冰'))
zhansan.marrry(new Man('男', '里斯'))
```
...
简单工厂（出现最多）： 一般就是一个函数返回产品类的实例。

工厂方法：多了工厂类，要想创建产品，需要先创建此工厂的实例，再通过此工厂来创建产品。
...
- 1.客户端 使用这个类的使用者必须知道这是一个单例的类，必须主动调用getInstance方法

- 2.并不能阻止客户端new Window();

解决这两个问题，期待什么？

- 1. 可以直接 new Window，并且是单例的
...
let Window=(function () {
    ...
    let Window=function (name) {
        if (window) {
            return window; // new 关键字，如果返回的是一个对象，那么返回的就是对象
        } else {
            this.name=name;
            return (window=this);
        }
...
let d1=new CreateWindow('zfpx');
let d2=new CreateWindow('zfpx');
console.log(d1 === d2)
```
...
document.getElementById('hide-btn').addEventListener('click',function (event) {
...
Redux 整个应用只有一个仓库，整个仓库只有一个状态 state
...
性能差，最好优化成O(1)的，可以用链表，超出尾部指针删除，头部插入，更新则更新至头部

![](http://cdn.wangtongmeng.com/20260103191756.png)
...
function mixins(...others) {
    return function (target) {
        Object.assign(target.proßtotype,...others);
    }
}
@mixins(Hooks)
class Component{
